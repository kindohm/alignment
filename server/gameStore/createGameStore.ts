...
export type CreateChartInput = {
  ...
  xAxisName: string;
  yAxisName: string;
  images: Array<Pick<ChartImage, "url" | "filename" | "contentType" | "width" | "height">>;
};
...
export const createGameStore = () => {
  ...
  const listCharts = (): Chart[] => [...charts.values()].sort((a, b) => a.name.localeCompare(b.name));

  const createChart = (input: CreateChartInput): Chart => {
    const timestamp = now();
    const chart: Chart = {
      id: createId("chart"),
      name: input.name,
      ...
      images: input.images.map((image, order) => ({
        id: createId("img"),
        storageKey: `local/${image.filename}`,
        url: image.url,
  ...
  const getGameBySlug = (slug: string): Game | undefined => {
  ...
  const createGame = (chartId: string): Game => {
  ...
  const joinGame = (slug: string, player: Pick<Player, "id" | "username">): Game => {
  ...
  const renamePlayer = (slug: string, playerId: string, username: string): Game => {
    const game = getGameBySlug(slug);
  ...
  const leaveGame = (slug: string, playerId: string): Game | undefined => {
    const game = getGameBySlug(slug);
  ...
  const startNextRound = (slug: string): Game => {
    const game = getGameBySlug(slug);
  ...
  const upsertVote = (slug: string, playerId: string, coordinate: Coordinate): Game => {
    const game = getGameBySlug(slug);
  ...
  const endRound = (slug: string): Game => {
    const game = getGameBySlug(slug);
